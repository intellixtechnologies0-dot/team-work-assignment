...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔍 DEBUG: DOM loaded, initializing debug app...');
    initializeApp();
    setupEventListeners();
    checkAuthStatus();
});
...
async function checkAuthStatus() {
    try {
        ...
        if (user && !error) {
            console.log('🔍 DEBUG: User is logged in:', user.email);
            currentUser = user;
            showMainApp();
...
function updateUIForUserRole(isAdmin) {
    console.log('🔍 DEBUG: Updating UI for user role. Is admin:', isAdmin);
    
    const addMemberBtn = document.getElementById('addMemberBtn');
    const createTaskBtn = document.getElementById('createTaskBtn');
    
    console.log('🔍 DEBUG: Add member button found:', !!addMemberBtn);
    console.log('🔍 DEBUG: Create task button found:', !!createTaskBtn);
    
    if (isAdmin) {
        // Admin can see all features
        if (addMemberBtn) {
            addMemberBtn.style.display = 'inline-flex';
            console.log('🔍 DEBUG: Showing add member button');
        }
        if (createTaskBtn) {
            createTaskBtn.style.display = 'inline-flex';
            console.log('🔍 DEBUG: Showing create task button');
        }
    } else {
        // Regular users can only view their tasks
        if (addMemberBtn) {
            addMemberBtn.style.display = 'none';
            console.log('🔍 DEBUG: Hiding add member button');
        }
        if (createTaskBtn) {
            createTaskBtn.style.display = 'none';
            console.log('🔍 DEBUG: Hiding create task button');
        }
    }
}
...
async function handleAddMember(event) {
    ...
    try {
        console.log('🔍 DEBUG: Adding team member:', { name, email, role });
        const { data, error } = await supabase
            .from('team_members')
            .insert([
                {
...
async function loadTeamMembers() {
    try {
        ...
        if (!window.isAdmin) {
            console.log('🔍 DEBUG: Not admin, skipping team members load');
            teamMembers = [];
            renderTeamMembers();
            return;
        }
        ...
        console.log('🔍 DEBUG: Team members loaded:', data?.length || 0);
        teamMembers = data || [];
        renderTeamMembers();
    } catch (error) {
        console.error('🔍 DEBUG: Error loading team members:', error);
...
async function handleAddTask(event) {
    ...
    try {
        console.log('🔍 DEBUG: Creating task:', { title, assignee: assignee.name });
        const { data, error } = await supabase
            .from('tasks')
            .insert([
                {
...
async function updateTaskStatus(taskId, newStatus) {
    try {
        console.log('🔍 DEBUG: Updating task status:', taskId, 'to', newStatus);
        const { error } = await supabase
            .from('tasks')
        ...
        console.error('🔍 DEBUG: Error updating task status:', error);
        showNotification(error.message, 'error');
    }
}
...
async function loadTasks() {
    try {
        ...
        if (!window.isAdmin) {
            console.log('🔍 DEBUG: Not admin, filtering tasks for user:', currentUser.email);
            ...
            if (memberError) {
                console.log('🔍 DEBUG: No team member record found for:', currentUser.email);
            ...
            console.log('🔍 DEBUG: Found team member ID:', memberData.id);
        ...
        console.log('🔍 DEBUG: Tasks loaded successfully:', data?.length || 0);
        tasks = data || [];
        renderTasks();
    } catch (error) {
...
function renderTasks() {
    console.log('🔍 DEBUG: Rendering tasks...');
    const todoTasks = tasks.filter(task => task.status === 'todo');
    const inProgressTasks = tasks.filter(task => task.status === 'inProgress');
    const completedTasks = tasks.filter(task => task.status === 'completed');
    
    console.log('🔍 DEBUG: Task counts - Todo:', todoTasks.length, 'In Progress:', inProgressTasks.length, 'Completed:', completedTasks.length);
